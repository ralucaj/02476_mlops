    ...
    def train(self):
        ...
        for e in range(epochs):
            ...
            for batch in range(len(train_set['images']) // batch_size):
                ...
                log_ps = model(train_set['images'][batch * batch_size:(batch + 1) * batch_size])
                loss = criterion(log_ps, train_set['labels'][batch * batch_size:(batch + 1) * batch_size])
                loss.backward()
                optimizer.step()
    ...
    def evaluate(self):
        ...
        with torch.no_grad():
            ...
            for batch in range(len(test_set['images']) // batch_size):
                log_ps = model(test_set['images'][batch * batch_size:(batch + 1) * batch_size])
                loss = criterion(log_ps, test_set['labels'][batch * batch_size:(batch + 1) * batch_size])
                running_loss += loss.item()
                log_ps = nn.Softmax()(log_ps)
                _, top_class = log_ps.topk(1, dim=1)
                top_class = top_class.squeeze()
                preds = torch.cat((preds, top_class))
                labels = torch.cat((labels, test_set['labels'][batch * batch_size:(batch + 1) * batch_size]))
